const cargarPosts=async()=>{
    ...
    data.map(item=>{
        ...
        <td>${item.title}</td>
        <td>${item.body}</td>
        <td><button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#editModal" onclick="cargarPost(${item.id})"> <i class="bi bi-pencil-square"></i> Editar</button></td>
        <td><button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal" onclick="eliminarPost(${item.id})"><i class="bi bi-trash-fill m-2"></i>Eliminar</button></td>
        </tr>`;
    })
}
